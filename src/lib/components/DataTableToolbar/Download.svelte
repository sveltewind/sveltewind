  ...
  export let confirmHandler = async () => {
    try {
      ...
      const body = selectedRows.map((row) =>
        [...headers].map((column) => row[column.key])
      );
      if (downloadType === 'Clipboard')
        download.clipboard(
          [headers.map(({ label }) => label), ...body]
            .map((row) => row.join('\t'))
            .join('\r\n')
        );
      if (downloadType === 'CSV')
        download.csv([headers.map(({ label }) => label), ...body]);
...
{#if active}
  ...
  <Modal bind:close bind:isOpen bind:open bind:toggle class="space-y-[1rem]">
    ...
    <div class="grid grid-cols-2 gap-[.5rem]">
      ...
      <Button on:click={confirmHandler}
        >{downloadType === 'Clipboard' ? 'Copy' : 'Download'}</Button
      >
    </div>
  </Modal>
{/if}
